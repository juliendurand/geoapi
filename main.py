...
if __name__ == '__main__':
    ...
    departement = 77
    number_csv_path = os.path.join(data_path, filename_template % departement)
    with open(number_csv_path, encoding='UTF-8') as in_file:
        next(in_file, None)  # skip header (first line)
        for line in in_file:
            values = line[:-1].replace('""', '').split(';')
            numero = values[3]
            ...
            x3, y3 = conv_wsg84_to_lambert93(lon, lat)

            print('------')
            print('position :', lon, lat)
            print('ban : ', x, y)
            print('pyproj : ', x3, y3)
            print('pyproj diff : ', x - x3, y - y3)
